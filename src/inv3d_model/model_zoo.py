...
from .score.score import score_all
...
class ModelZoo:
    ...
    def load_model(self, name: str) -> LightningModule:
        ...
        if len(checkpoints) == 0:
            print(
                f"Error! No checkpoints found in directory {(self.root_dir / name).as_posix()}!"
            )
            exit()
    ...
    def train_model(
        ...
        if not resume:
            shutil.copyfile(
                inspect.getsourcefile(model.__class__), output_dir / "model.py"  # type: ignore
            )
        ...
        if patience:
            callbacks.append(
                EarlyStopping(
                    monitor="val/mse_loss", patience=patience, mode="min", verbose=True
                )
            )
    ...
    def eval_model(
        self, trained_model: str, dataset: str, gpu: int, num_workers: int = 4
    ):
        ...
        eval_dataset = self.dataset_factory.create(
            name=dataset, split=DatasetSplit.TEST, **model.dataset_options
        )
        ...
        with tqdm.tqdm(total=len(eval_loader), desc="Inference samples") as progress:
            for index, batch in enumerate(eval_loader):
                ...
                norm_image = apply_map_torch(
                    image=orig_image, bm=out_bm, resolution=true_image.shape[-2:]
                )
                ...
                if index < 100:
                    ...
                    save_image(
                        output_dir / "orig_image.png", to_numpy_image(orig_image)
                    )
                    save_image(
                        output_dir / "true_image.png", to_numpy_image(true_image)
                    )
                    save_image(
                        output_dir / "norm_image.png", to_numpy_image(norm_image)
                    )
    ...
    def inference(self, name: str, dataset_name: str, gpu: int, num_workers: int = 4):
        ...
        inference_dataset = self.dataset_factory.create(
            name=dataset_name, split=DatasetSplit.TEST, **model.dataset_options
        )
        ...
        with tqdm.tqdm(
            total=len(inference_loader), desc="Inference samples"
        ) as progress:
    ...
    def show_sample(self, name: str, dataset_name: str, idx: int):
        example_dir = (
            self.root_dir / name / "eval" / dataset_name / "examples" / str(idx)
        )
        ...
        if (example_dir / "true_bm.npz").is_file():
            visualize_bm(
                example_dir / "orig_image.png",
                example_dir / "true_bm.npz",
                title="True backward map",
            )

        visualize_bm(
            example_dir / "orig_image.png",
            example_dir / "out_bm.npz",
            title="Output backward map",
        )
