...
from .mapping import create_identity_map, scale_map
...
def visualize_matplotlib2(
    data1: np.ndarray, data2: np.ndarray, return_image: bool = False
):
    ...
    cax = fig.add_axes([0.85, 0.356, 0.025, 0.295])
...
def visualize_bm(
    image: Union[Path, np.ndarray, torch.Tensor],
    bm: Union[Path, np.ndarray],
    resolution: int = 16,
    title: str = None,
...
def visualize_sequence(
    *,
    ...
    def create_frame(frame_idx, state):
        ...
        mesh_texts = [
            "index:   (y: {},        x: {})<br>values: (y: {:4.3f}, x: {:4.3f})".format(
                py, px, data[py, px, 0], data[py, px, 1]
            )
            for py in range(resolution)
            for px in range(resolution)
        ]
        mesh_points = go.Scatter(
            x=data[:, :, 1].reshape(-1),
            y=data[:, :, 0].reshape(-1),
            mode="markers",
            name="",
        ...
        if show_gt:
            ...
            arrow_texts = [
                "(dy: {:.3f}, dx: {:.3f})".format(dy, dx)
                for dy, dx in shift.reshape(-1, 2)
            ]
            arrow_points = [
                go.Scatter(
                    x=new_data[:, :, 1].reshape(-1),
                    y=new_data[:, :, 0].reshape(-1),
                    mode="markers",
                    name="",
                    marker=dict(color=arrowcolor),
                    opacity=0,
                    text=arrow_texts,
                    hoverinfo="text",
                )
        ...
        return go.Frame(
            data=lines1 + lines2 + [mesh_points] + arrow_points,
            layout=go.Layout(dict(annotations=arrows)),
            name=f"frame_{frame_idx}",
        )

    frames = [
        create_frame(frame_idx, state) for frame_idx, state in enumerate(state_sequence)
    ]
